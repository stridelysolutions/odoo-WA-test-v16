...
import { MassMailingMobilePreviewDialog } from "@mass_mailing/js/mass_mailing_mobile_preview";
...
export class WhatsappHtmlField extends HtmlField {
    setup() {
        super.setup();
        useSubEnv({
    ...
    async _onSnippetsLoaded() {
        ...
        var mailing_model_id = ''
        if(this.props.record.data.mailing_model_id){
            mailing_model_id = this.props.record.data.mailing_model_id[1]
        }
        else{
            mailing_model_id = ''
        }
...
WhatsappHtmlField.displayName = _lt("Email");
WhatsappHtmlField.extractProps = (...args) => {
    const [{ attrs }] = args;
...
registry.category("fields").add("whatsapp_html", WhatsappHtmlField);
